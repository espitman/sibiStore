...
import type { App, Platform, State, Version } from './types';
import './style.css';
...
type PlatformFilter = 'All' | 'Phone' | 'TV' | 'VR';
const versionPlatform = (version: Version): Platform => version.platform || (version.vr ? 'vr' : version.tv ? 'tv' : 'phone');
const matchesPlatform = (version: Version, filter: PlatformFilter) => filter === 'All' || versionPlatform(version) === filter.toLowerCase();
...
function Desktop() {
  ...
  const platformApps = (state?.apps || []).flatMap(a => {
    const versions = a.versions.filter(v => matchesPlatform(v, platform));
      ...
      {page === 'Library' && <main className="library-page">
        ...
        <div className="library-columns">
          <div className="library-main">
            ...
            <div className="platform-filters" role="group" aria-label="Filter by platform">{(['All', 'Phone', 'TV', 'VR'] as const).map(value => <button key={value} aria-pressed={platform === value} className={platform === value ? 'active' : ''} onClick={() => { setPlatform(value); setDetails(false); }}>{value}<span>{(state?.apps || []).filter(a => a.versions.some(v => matchesPlatform(v, value))).length}</span></button>)}</div>
            <div className="table-scroll"><table><thead><tr><th>App</th><th>Latest version</th><th>Size</th><th>Status</th></tr></thead><tbody>{apps.map(a => <tr key={a.packageName} className={a.packageName === app?.packageName ? 'selected' : ''} tabIndex={0} onClick={() => { setSelected(a.packageName); setDetails(false); }} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelected(a.packageName); } }} aria-selected={a.packageName === app?.packageName}><td><div className="app-cell"><AppIcon app={a}/><div><strong>{a.title}</strong><small title={a.packageName}>{a.packageName}</small></div></div></td><td>{a.versions[0].versionName || a.versions[0].versionCode}</td><td>{size(a.versions[0].size)}</td><td><span className="ready"><i className="dot"/>Ready</span></td></tr>)}</tbody></table>
              {!apps.length && <div className="empty"><FileBox size={42}/><h2>{query ? 'No matching apps' : platform !== 'All' ? `No ${platform} apps` : 'Your library starts here'}</h2><p>{query ? 'Try another app name or package.' : platform !== 'All' ? 'Choose All or copy matching APKs into your folder.' : 'Copy APKs into your folder. Sibi Store will take care of the rest.'}</p>{!query && <button className="outline-button" onClick={() => invoke(() => window.sibi.openFolder())}><Folder size={18}/>Open APK folder</button>}</div>}
            </div>
          ...
          <aside className="inspector">{app && version ? <><div className="inspector-heading"><AppIcon app={app} large/><div><h2>{app.title}</h2><p className="package" title={app.packageName}>{app.packageName}</p><span className="ready"><i className="dot"/>Ready to serve</span></div></div>
            <h3>Version history</h3><div className="version-list">{app.versions.map((v,i) => <div className="version-entry" key={v.sha256}><div className="version-title">{v.versionName || v.versionCode}{i === 0 && <span className="latest">Latest</span>}</div><p>Version code {v.versionCode} · {size(v.size)}</p><div className="version-file"><span title={v.filename}>{v.filename}</span><time>{date(v.addedAt)}</time></div><label className="version-platform"><span>Platform</span><select aria-label={`Platform override for ${v.versionName || v.versionCode}`} value={v.platformOverride || 'auto'} onChange={e => invoke(async () => setState(await window.sibi.setPlatformOverride(v.sha256, e.target.value as 'auto' | Platform)))}><option value="auto">Auto</option><option value="phone">Phone</option><option value="tv">TV</option><option value="vr">VR</option></select></label></div>)}</div>
            <h3>Compatibility</h3><div className="info-row"><span>Platform</span><span>{versionPlatform(version) === 'phone' ? 'Phone' : versionPlatform(version).toUpperCase()}</span></div><div className="info-row"><span>Minimum SDK</span><span>API {version.minSdk}</span></div><div className="info-row"><span>Architecture</span><span>{version.abis.length ? version.abis.join(', ') : 'Universal'}</span></div>
